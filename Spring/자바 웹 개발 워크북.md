...
인스턴스 직접 생성: Tire -> new -> 인스턴스

인스턴스 간접 생성: Tire -> TireFactory -> 인스턴스

* 팩토리 클래스는 인스턴스 생성과정을 캡슐화 한다.
* 팩토리 메서드는 **<u>스태틱으로 선언하여 클래스 메서드</u>**로 만들거나, **<u>인스턴스 메서드</u>**로 만드는 방법이 있다.
...
public class TireFactory {
    // 팩토리 메서드, 메개변수 값으로 제조사 이름을 주면 해당 제조사의 다이어를 생산하는 메서드
...
* createTire("Hankook") -> TireFactory -> Tire 인스턴스
...
TireFactory 클래승에서 모든 메서드의 static 선언을 제거, 즉 모든 메서드를 인스턴스 메서드로 만든다.
...
* `AbstractFactoryBean`에는 createInstance() 라는 추상 메서드가 있다. 빈을 생성할 때 팩토리 메서드로서 getObject()가 후출되는데, getObject()는 내부적으로 바로 이 메서드를 호출한다.

  즉 실제 빈을 생성하는 것은 createInstance() 이다. 따라서 `AbstractFactoryBean` 클래스를 상속받을 때는 반드시 이 메서드를 구현해야 하고, 이 메서드에 빈 생성 코드를 넣는다.

* `public class TireFactoryBean extends AbstractFactoryBean<Tire> {`: TireFactoryBean  클래스는 AbstractFactoryBean 클래스를 상속받는다. AbstractFactoryBean은 하나의 코드로 다양한 타입의 빈을 다룰 수 있도록 제너릭 문법이 적용된 클래스이다. 

  이 클래스를 상속받을 때 타입 매개변수 `T(AbstractFactoryBean<T>)`에 들어갈 클래스 타입을 지정한다. 즉 팩토리 빈이 생성해 줄 인스턴스의 타입을 지정한다. 위에서는 Tire 객체를 지정했다.
...
싱글톤:  한개의 인스턴스만 생성하여 사용

프로토타입: 매번 요청할 때마다 인스턴스를 생성
...
<!-- 
빈의 범위를 설정하지 않으면 싱글톤, or scope="singleton"
-->
	<bean id="kiaEngine" class="exam.test14.Engine" p:maker="Kia" p:cc="3000" scope="prototype"/>
<!--
빈 컨테이너에게 kiaEngine을 요청할 때마다 매번 새 Engine인스턴스를 만들어 반환한다.
-->
```
...
`<bean ... p:프로퍼티명="값" />`: 빈 설정파일은 xml이기 때문에 결국 프로퍼티 값은 문자열로 표현한다.

문자열은 숫자로 변환하기 쉽기 때문에 숫자 타입 프로퍼티인 경우 별도의 추가 작업없이 간단히 값을 설정할 수 있으나 그 외 타입은 자동으로 변환되지 않으므로 개발자가 직접 프로퍼티 타입에 맞는 객체를 생성해야 한다.
...
<!-- 
parse()는 문자열을 가지고 날짜 객체를 만들어 주는 인스턴스 메서드

hankookTire.setCreateDate(dateFormat.parse("2014-5-5"));
-->
```
...
앞의 방식은 날짜 프로퍼티 값을 설정할 때마다 팩토리 메서드 빈을 선언해야 하는 점이 문제다. 이런 불편함을 해결하기 위해 스프링 IoC 컨테이너는 프로퍼티 에디터를 도입했다.

프로퍼티 에디터(Property Editor): 문자열을 특정 타입의 값으로 변환해주는 객체
...
프로퍼티 편집기 CustomDateEditor를 생성한다. 문자열을 날짜 객체로 바꿔줄 빈(dateEditor)을 설정
-->
...
만든 프로퍼티 에디터 등록기 CustomPropertyEditorRegistrar 인스턴스를 준비하고 날짜 값을 처리할 프로피티 에디터를 설정한다.
-->
...
CustomEditorConfigurer를 활용하여 프러퍼티 에디터 등록기를 설정
프로퍼티 에디터 등록기는 목록(List)에 담아서 propertyEditorRegistrars 속성값으로 지정한다.
-->
...
빈의 셋터 메서드에 @Autowired를 선언하면 빈 컨테이너가 셋터의 매개변수 타입과 일치하는 빈을 찾아 자동으로 설정해준다.

`<bean class="org.springframework.beans.factory.annotation.AutowiredAnnotationBeanPostProcessor" />`

`AutowiredAnnotationBeanPostProcessor`클래스는 빈의 후 처리기로서 빈을 생성한 후 즉시 @Autowired로 선언된 셋터를 찾아서 호출하는 역할을 수행한다. 즉 @Autowired가 붙은 프로퍼티에 대해 의존 객체를 찾아 주입하는 일을 한다.
...
```xml
  <bean class="org.springframework.beans.factory.annotation.AutowiredAnnotationBeanPostProcessor" />
<!-- 
AutowiredAnnotationBeanPostProcessor temp.AutowiredPostProcessor = new AutowiredAnnotationBeanPostProcessor();
-->
...
프로퍼티(셋터 메서드)에 대해 @Autowired를 선언하면 해당 프로퍼티는 기본으로 필수 입력 항목이 된다. 만약 플퍼티에 주입할 의존 객체를 찾을 수 없다면 예외가 발생한다. @Autowired의 require 속성을 false로 설정하면 프로퍼티에 주입할 의존 객체를 찾지 못하더라도 예외가 발생하지 않는다.
...
@Autowired는 프로퍼티에 주입할 수 있는 의존 객체가 여러 개 있을 경우 오류를 발생시킨다. 즉 같은 타입의 객체가 여러개일 경우 그 중에서 어떤 객체를 주입해야 할지 알 수 없는데, 이때 @Qualifier를 사용한다.
...
* `<context:annotation-config />`: 이 태그를 선언하면 애노테이션 처리를 담당하는 다음의 빈들이 자동 등록된다.

| 애노테이션                             | 빈의 후 처리기                       |
| -------------------------------------- | ------------------------------------ |
| @Autowired, @Value, @Inject(JSP-330)   | AutowiredAnnotationBeanPostProcessor |
| JSR-250 애노테이션(javax.annotation.*) | CommonAnnotationBeanPostProcessor    |
| @PersistenceUnit, @PersistenceContext  | PersistenceAnnotationBeanPreocessor  |
| @Requred                               | RequiredAnnotationBeanPostProcessor  |
...
JSR-250 명세에 정의된 @Resource를 사용하면 더 간단히 특정 의존 객체를 지정할 수 있다. 단, @Autowired는 required 속성을 사용하여 프로퍼티 값의 필수 여부를 지정할 수 있지만 @Resource에는 그런 기능이 없으므로 @Resource는 필수 입력이다.
...
컴포넌트 스캔과 애노테이션으로 빈을 자동 등록한다.

#### @Component가 붙은 클래스를 자동으로 찾기
...
* @Component에 들어가는 속성값은 빈의 이름이다. getBean()을 호출 할 때 이 이름을 사용한다.
...
* `<context:component-scan />` 태그르 사용하여 @Component, @Repository 등 빈 생성 표시자(애노테이션)가 붙은 클래스를 검색하라고 선언한다.
...
<!--
트랜잭션 관리를 담당할 객체를 선언한다.
DB 커넥션에 대해 트랜잭션 관리를 수행하는 것이기 때문에, dataSource 프로퍼티를 설정해야한다. 톰캣에서 가져온 DataSource 객체를 할당한다.
-->
...
<!--
SqlSessipn sqlSession = sqlSessionFactory.openSession();
...
* `<context:component-scan base-package="spms"/>`: 클래스 경로를 뒤져서 @Component 애노테이션이 붙은 클래스를 찾아 객체를 생성하게 하기 위해 선언한다.
  클래스를 조사할 폴더는 spms 패키지이다. 스프링 IoC컨테이너는 spms패키지를 포함하여 하위 패키지까지 모두 조사한다. 그리고 @Component 애노테이션이 붙은 클래스에 대해 인스턴스를 생성한다.

* `<property name="dataSource" ref="dataSource" />`: `mybatis-config.xml` 설정 파일이 아닌 SqlSessionFactoryBean의 프로퍼티 설정으로 대신한다.
* `<property name="typeAliasesPackage" value="spms.vo" />`: `<typeAliases>`의 값 ㄱ개체 정보는다음과 같이 typeAliasesPackage 프로퍼티 값으로 대신한다. 프로퍼티값 spms.vo는 값 객체가 들어있는 패키지 이름이다.
* `<property name="mapperLocations" value="classpath*:spms/dao/*Dao.xml" />`: `<mapper>`의 SQL 맵퍼 파일 정보는 다음과 같이 mapperLocations 프로퍼티 값으로 대신한다. `classpath*:spms/dao/*Dao.xml`패키지에서 파일의 이름이 Dao.xml로 끝나는 맵퍼 파일을 가리킨다.
...
public class ContextLoaderListener implements ServletContextListener {
  ...
  public void contextInitialized(ServletContextEvent event) {
    try {
      ...
      // 웹 애플리케이션을 시작하면 contextInitialized()가 호출될 것이고, 이 메서드에서는 스프링IoC컨테이너(ClassPathXmlApplicationContext)를 준비할 것이다. 스프링 IoC컨테이너는 beans.xml에 선언된 대로 데이터 소스와 트랜젝션 관리자, mybatis 객체를 생성하고, 클래스 경로에서 DAO와 페이지 컨트롤러를 찾아 인스턴스를 생성할 것이다.
    } catch(Throwable e) {
      e.printStackTrace();
    }
  }
...
* ```java
  ...
  ApplicationContext 대신에 스프링에서 제공하는 IoC컨테이너를 사용하기 때문에 클래스 변수와 겟터 메서드를 이에 맞추어 변경한다.
...
* spms.annotation.Component 클래스
* spms.annotation 패키지
* spms.context.ApplicationContext 클래스
* mybatis-config.xml
* db.properties 파일 삭제
